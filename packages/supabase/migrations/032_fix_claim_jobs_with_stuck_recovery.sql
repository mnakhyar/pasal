...
--
-- Fix: claim_jobs() now reclaims stale crawling jobs (>15 min) alongside pending ones,
-- with SET LOCAL statement_timeout to survive transient DB pressure.

-- 1. Self-healing claim_jobs: reclaims stale + claims pending in one query
...
BEGIN
    ...
    WHERE id IN (
        ...
        WHERE status = 'pending'
           OR (status = 'crawling' AND updated_at < NOW() - INTERVAL '15 minutes')
        ORDER BY
